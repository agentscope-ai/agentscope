...
- [X] T010 [US1] Add typed marker file `src/agentscope/py.typed`
- [X] T011 [US1] Ensure packaging includes `py.typed` by updating `setup.py` (`package_data` / include rules)
- [X] T012 [US1] Add/adjust test to assert `py.typed` exists via `importlib.resources` (e.g., in `tests/`)
...
- [X] T020 [US2] Add `src/agentscope/rag/_store/_milvuslite_store.py` (from local `main`, minimal adaptation)
- [X] T021 [US2] Export `MilvusLiteStore` in `src/agentscope/rag/_store/__init__.py`
- [X] T022 [US2] Export `MilvusLiteStore` in `src/agentscope/rag/__init__.py`
- [X] T023 [US2] Update `setup.py` extras to include `pymilvus[milvus_lite]` (NOT in minimal requires)
- [X] T023a [US2] Ensure `pymilvus[milvus_lite]` uses marker `platform_system != "Windows"` (keep Windows install green)
- [X] T024 [US2] Add a test that `import agentscope.rag` does not eagerly import `pymilvus`
  (assert `"pymilvus" not in sys.modules` after import)
- [X] T025 [US2] Add a test that `MilvusLiteStore` is a `VDBStoreBase` subclass (no runtime connection required)
...
- [X] T030 [US3] Add `src/agentscope/rag/_reader/_word_reader.py` (from local `main`, minimal adaptation)
- [X] T031 [US3] Export `WordReader` in `src/agentscope/rag/_reader/__init__.py`
- [X] T032 [US3] Export `WordReader` in `src/agentscope/rag/__init__.py`
- [X] T033 [US3] Update `setup.py` extras to include `python-docx` (NOT in minimal requires)
- [X] T034 [US3] Add a test that `import agentscope.rag` does not eagerly import `docx`
  (assert `"docx" not in sys.modules` after import)
- [X] T035 [US3] Add minimal WordReader functionality test: pytest generates a `.docx` at runtime (no binary fixture),
...
- [X] T040 Run `PYTHONDONTWRITEBYTECODE=1 ./.venv/bin/python -m pytest -q -p no:cacheprovider tests/rag_reader_test.py tests/rag_store_test.py`
- [X] T041 Run `./.venv/bin/python -m ruff check src`
- [X] T042 Commit with message like: `rag: absorb main tier-A additions (py.typed + milvus + word reader)`
