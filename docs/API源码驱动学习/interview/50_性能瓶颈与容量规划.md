...
## 3.1 per-session Agent 构建缓存（fork 独有增强）

> 来源：`src/agentscope/app/_service/_chat.py`（提交 `21057798`），属于本 fork 相对 upstream 的独有优化，官方文档未覆盖。

**问题**：每个 chat turn 默认会重新构建 `toolkit + model + agent` 对象图。Agent 对象里持有 model client、toolkit（含多个 MCP client）、memory 等重对象，重建成本包括：MCP client 重连、toolkit 重新 `list_tools()`、model client 重新实例化。高频对话或高并发下这会成为显著的 CPU 与延迟浪费。

**解法**：`_chat.py` 增加 **per-session agent build cache**——以 `(agent_id, session_id)` 为 key 的 LRU 缓存，命中则直接复用上一次构建好的 agent 对象，跳过重构建；未命中才重建并写入缓存。

| 维度 | 说明 |
|------|------|
| 缓存 key | `(agent_id, session_id)`——同一会话连续多轮复用，换会话不串 |
| 淘汰策略 | LRU（最近最少使用）；配合 per-session 生命周期清理，session 关闭/reset 时失效对应条目 |
| 收益 | 省去 MCP 重连 + `list_tools()` + model client 重建；典型多轮对话每轮省一次 agent 装配 |
| 风险点 | agent 内持有的对象若有会话外可变状态需注意隔离；reset / 换 model 配置时必须让缓存失效（否则用到陈旧对象） |
| 可观测 | 命中率应作为容量指标之一（见 §7 监控指标清单） |

面试表达：

```text
Agent 构建本身有成本——MCP 重连、list_tools、model client 实例化都耗时。我做了一层 per-session 的 LRU 缓存，同一个会话连续多轮直接复用 agent 对象，避免每轮重构建。缓存按 (agent_id, session_id) 隔离，session reset 或模型配置变化时才失效，不会串数据。
```
