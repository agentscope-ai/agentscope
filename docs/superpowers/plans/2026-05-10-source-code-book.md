...
3. **写作** — 按 spec 定义的章节内部结构撰写，所有代码段标注真实行号范围。每章至少包含：1 个"试一试"环节、1 个 Mermaid 图表、1 个"官方文档对照"侧边栏、1 个"推荐阅读"链接。卷一和卷二每章至少 1 个"设计一瞥"侧边栏
4. **自检** — 对照 spec 检查：(a) 章节内部结构元素是否齐全 (b) "试一试"是否可执行（含无 API key 替代方案） (c) Mermaid 图表是否存在 (d) 设计一瞥侧边栏是否存在（卷一、卷二） (e) 行号是否通过 `sed -n` 验证 (f) "官方文档对照"侧边栏是否存在
