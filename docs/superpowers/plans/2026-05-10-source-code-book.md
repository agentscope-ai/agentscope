...
1. **源码验证**（卷一~四）— `grep -n` / `sed -n` 确认所有将引用的文件路径、类名、方法名、行号
...
3. **写作** — 按 spec 定义的章节内部结构撰写，所有代码段标注真实行号范围。每章至少包含：1 个"试一试"环节、1 个 Mermaid 图表。卷一和卷二每章至少 1 个"设计一瞥"侧边栏。**所有参考资料（论文、官方文档、教程）直接融入正文**，不使用侧边栏+链接格式。论文用引用格式摘录 1-3 句并标注出处，官方文档的用法示例直接写进正文或"试一试"，PEP/技术规范条款摘录到"知识补全"
4. **自检** — 对照 spec 检查：(a) 章节内部结构元素是否齐全 (b) "试一试"是否可执行（含无 API key 替代方案） (c) Mermaid 图表是否存在 (d) 设计一瞥侧边栏是否存在（卷一、卷二） (e) 行号是否通过 `sed -n` 验证 (f) 参考资料是否全部融入正文（无悬空的外部链接侧边栏）
5. **提交** — `git add` + `git commit`
...
- `call_tool_function` at line 853
...
**源码验证目标：**
- `ReActAgent.reply` at line 376
- `_reasoning` at line 540
- `_acting` at line 657
- `_summarizing` at line 725
- `_compress_memory_if_needed` at line 1015
- `PlanNotebook` at line 172
...
**1. Spec 覆盖：** 36 章 + 3 附录 + 1 README = 40 个文件，全部映射到 Task 0-38。spec 中的每个章节都有对应任务。
...
- ch10 补充 `tool/_async_wrapper.py` 和 `tool/_types.py`
- ch11 补充 `agent/_react_agent_base.py`
- ch14 开场场景统一
...
- 自检流程扩展为 5 项检查（含行号验证、设计一瞥、无 key 替代方案）
- Task 0 增加旧文件备份步骤
- 文件总数修正为 40

**4. 源码引用一致性：** 关键行号需在写作时验证。
