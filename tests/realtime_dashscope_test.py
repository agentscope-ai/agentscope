...
class DashScopeCardsTest(unittest.TestCase):
    ...
    def test_omni_cards(self) -> None:
        self.assertListEqual(
            ...
            [
                ("qwen-omni-turbo-realtime", "dashscope_omni_realtime", False, None),
                ("qwen3-omni-flash-realtime", "dashscope_omni_realtime", False, 8),
                ("qwen3.5-omni-flash-realtime", "dashscope_omni_realtime", True, 80),
                ("qwen3.5-omni-plus-realtime", "dashscope_omni_realtime", True, 100),
            ],
        )
    ...
    def test_audio_cards(self) -> None:
        self.assertListEqual(
            [
                (c.name, c.model_type, c.max_audio_turns, c.max_audio_duration_s)
                for c in DashScopeAudioRealtimeModel.list_models()
            ],
            [
                ("qwen-audio-3.0-realtime-flash", "dashscope_audio_realtime", 50, 300),
                ("qwen-audio-3.0-realtime-plus", "dashscope_audio_realtime", 50, 300),
            ],
        )
...
class DashScopeSessionUpdateTest(unittest.TestCase):
    ...
    def test_turn_detection_none_hands_endpointing_to_caller(self) -> None:
        ...
        self.assertIsNone(model._session_update("x", None)["session"]["turn_detection"])
    ...
    def test_adapter_facts(self) -> None:
        ...
        audio = DashScopeAudioRealtimeModel("qwen-audio-3.0-realtime-plus", CRED)
        self.assertListEqual(
            [
...
class DashScopeParseTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self.model = DashScopeRealtimeModel("qwen3.5-omni-flash-realtime", CRED)
        self.model._parse({"type": "response.created", "response": {"id": "r1"}})

    def test_reply_frames(self) -> None:
        frames = [
            {"type": "response.audio_transcript.delta", "delta": "你好"},
            {"type": "response.audio.delta", "delta": base64.b64encode(b"\x01\x00").decode()},
            {"type": "input_audio_buffer.speech_started", "item_id": "u1", "audio_start_ms": 120},
            {"type": "conversation.item.input_audio_transcription.completed", "item_id": "u1", "transcript": "天气"},
            {"type": "response.done", "response": {"id": "r1", "usage": {"input_tokens": 10, "output_tokens": 5}}},
        ...
        self.assertListEqual(
            ...
            [
                ...
                me.AudioDeltaEvent(item_id="r1", pcm=b"\x01\x00", sample_rate=24000),
                me.SpeechStartedEvent(item_id="u1", at_ms=120),
                me.InputTranscriptionEvent(item_id="u1", text="天气"),
                me.ResponseDoneEvent(item_id="r1", input_tokens=10, output_tokens=5),
                None,
            ],
        )
    ...
    def test_tool_call_done_frame_is_authoritative(self) -> None:
        ...
        self.model._parse({"type": "response.function_call_arguments.delta", "call_id": "c1", "delta": '{"city":'})
        with_args = self.model._parse(
            {"type": "response.function_call_arguments.done", "call_id": "c1", "arguments": '{"city":"sh"}'},
        )
        self.model._parse({"type": "response.function_call_arguments.delta", "call_id": "c2", "name": "f", "delta": '{"a":1}'})
        without_args = self.model._parse({"type": "response.function_call_arguments.done", "call_id": "c2"})
    ...
    def test_audio_specific_frames_are_known_and_dropped(self) -> None:
        model = DashScopeAudioRealtimeModel("qwen-audio-3.0-realtime-plus", CRED)
        self.assertListEqual(
            [
                model._parse({"type": "conversation.item.ambient_audio_transcription.delta", "delta": "x"}),
                model._parse({"type": "voiceprint_audio_list.completed"}),
                model._parse({"type": "error", "error": {"code": "E1", "message": "boom"}}),
            ],
