...
itself. Missing an event or replaying one twice is therefore a lost or
duplicated reply, so the seam between the replay log and the live
subscription is what these cover.
"""
import asyncio
...
async def _publish(
    ...
    await publish_session_event(bus, session_id, event.model_dump(mode="json"))
...
class _SeamBus(InMemoryMessageBus):
    ...
    That reproduces the one case the stream deduplicates: an event that
    arrives after the subscription opened but before the replay
...
class EventStreamTest(IsolatedAsyncioTestCase):
    """The stream is gap-free across the replay/live seam."""
    ...
    async def test_seam_events_are_not_delivered_twice(self) -> None:
        ...
        This is the whole reason the stream tracks entry ids, and the
        window is too narrow to hit by timing, so the bus forces it.
        """
        bus = _SeamBus()
        await _publish(bus, "s-1", _start())
