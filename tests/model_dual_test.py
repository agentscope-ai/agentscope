# -*- coding: utf-8 -*-
"""Unit tests for DeepSeek model with DeepSeekChatFormatter."""
...
class TestDeepSeekModel(IsolatedAsyncioTestCase):
    """Test cases for DeepSeek model with DeepSeekChatFormatter."""
    ...
    def _create_agent(name, sys_prompt, model=None, toolkit=None, **kwargs):
        """Create a ReActAgent with DeepSeekChatFormatter."""
        return ReActAgent(
            name=name,
            sys_prompt=sys_prompt,
            model=model or TestDeepSeekModel._create_model(),
    ...
    async def test_react_agent_coding(self) -> None:
        """Test ReActAgent with code execution tool."""
        toolkit = Toolkit()
    ...
    async def test_react_agent_custom_tools(self) -> None:
        ...
        msg1 = Msg(
            name="user",
            content="北京今天天气怎么样？帮我算一下 123 乘以 456",
            role="user",
        )
    ...
    async def test_sequential_pipeline(self) -> None:
        ...
        seq_result = await seq(
            Msg(name="user", content="AI 是什么？", role="user")
        )
    ...
    async def test_fanout_pipeline(self) -> None:
        ...
        fanout_results = await fanout(
            Msg(name="user", content="解释一下什么是 AI Agent", role="user")
        )
