...
class TestOpenSandboxWorkspaceManager(IsolatedAsyncioTestCase):
    """Manager cache, metadata, use_server_proxy and volumes forwarding."""
    ...
    async def test_get_workspace_forwards_all_config_including_opts(
        self,
    ) -> None:
        """Manager forwards base config, use_server_proxy and volumes."""
        volumes = [
            {
                "type": "bind",
                "source": "/srv/data",
    ...
    async def test_get_workspace_uses_workspace_id_cache_key(self) -> None:
        """Same workspace id returns cached instance."""
        manager = OpenSandboxWorkspaceManager()
    ...
    async def test_safe_close_swallows_workspace_close_errors(self) -> None:
        ...
        await OpenSandboxWorkspaceManager._safe_close(workspace)
