...
- `on_check_permission` 在工具查找和输入校验之后、Agent 消费权限决策之前
  运行。
- 审计中间件调用一次 `next_handler()`，记录最终的 ASK/DENY/ALLOW 决策，
  并原样返回同一个决策对象。
...
trace，也不提供被压制的中间候选决策。用户确认输入属于另一个生命周期阶段；
应用如需观测，可通过 `on_reply` 处理。
...
审计中间件调用 `next_handler()` 时，内置权限引擎仍是决策来源。最终结果可能
受 permission mode 和已配置规则影响，因此可能不同于工具最初给出的建议；
本示例只记录最终结果。
