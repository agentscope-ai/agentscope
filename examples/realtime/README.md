...
Both accept audio + text, support tools, and use 16 kHz capture / 24 kHz
playout. Audio-only sessions are capped at 15 minutes; the adapter resumes
with a session-resumption handle. Default voice is Puck (see the card
parameter_overrides.voice enum for the full list).
