...
ch_names ={"werewolf": "狼人", "villager": "村民", "seer": "预言家", "witch": "女巫", "hunter": "猎人", "guard": "守卫", "wolf_king": "狼王"}

# 发言顺序配置
# 可选值: "from_death" (从死者位置开始), "reverse" (逆序), "original" (原顺序)
SPEAKING_ORDER_TYPE = "from_death"

async def notify_hunter_poison_status(hunter_agent: ReActAgent, is_poisoned: bool) -> None:
    """夜晚告知猎人是否中毒"""
    global moderator
    if is_poisoned:
...
async def notify_wolf_king_poison_status(wolf_king_agent: ReActAgent, is_poisoned: bool) -> None:
    """夜晚告知狼王是否中毒"""
    global moderator
    if is_poisoned:
...
async def handle_self_explode(agent: ReActAgent, all_players_hub: MsgHub) -> bool:
    """处理狼人自爆逻辑，返回是否自爆"""
    global moderator
...
async def handle_last_words(all_players_hub: MsgHub, dead_players: list[str], is_voted: bool = False, is_gunned: bool = False) -> None:
    """处理遗言逻辑"""
    global moderator, all_players_original_order
...
async def choose_sheriff_speaking_order_for_discussion(all_players_hub: MsgHub, sheriff: str, has_death: bool) -> bool:
    """警长选择讨论发言顺序（有死亡或平安夜时）"""
    global moderator, sheriff_speaking_direction
    ...
    if sheriff_agent:
        ...
        direction = "死右" if (has_death and sheriff_speaking_direction) else "死左" if (has_death and not sheriff_speaking_direction) else "警右" if (not has_death and sheriff_speaking_direction) else "警左"
        await all_players_hub.broadcast(
...
async def choose_sheriff_speaking_order(all_players_hub: MsgHub, sheriff: str) -> None:
    """警长选择发言顺序"""
    global moderator, sheriff_speaking_direction
...
async def sheriff_election_vote(all_players_hub: MsgHub, all_players: list[ReActAgent]) -> bool:
    """第二天直接投票选举警长（双爆吞警徽逻辑）"""
    ...
    exploded_agent = None
    for agent in all_players:
        if await handle_self_explode(agent, all_players_hub):
    ...
    if exploded_agent:
        ...
        # 重置标志位
        first_explode_interrupted_election = False
        return False
...
async def sheriff_election(all_players_hub: MsgHub, all_players: list[ReActAgent]) -> bool:
    ...
    # 只有未上警的玩家可以投票
    voting_agents = [agent for agent in all_players if agent.name not in first_day_candidates]
    ...
    if is_tie:
        ...
        # PK发言
        await all_players_hub.broadcast(
            await moderator(Prompts.to_all_election_pk.format(names_to_str(tied_candidates))),
        )
        
        pk_candidates = [agent for agent in active_candidates if agent.name in tied_candidates]
        ...
        voting_agents = [agent for agent in current_alive if agent.name not in tied_candidates]
        ...
        if pk_is_tie:
            # PK也平票，警徽流失
            await all_players_hub.broadcast(
                await moderator(Prompts.to_all_election_pk_tie.format(pk_votes, names_to_str(tied_candidates))),
            )
            return False
        else:
            # PK有结果
            ...
            sheriff_has_badge = True
            await all_players_hub.broadcast(
                await moderator(Prompts.to_all_election_pk_result.format(pk_votes, sheriff)),
            )
            # 警长选择发言顺序
            await choose_sheriff_speaking_order(all_players_hub, sheriff)
    else:
...
async def handle_sheriff_death(dead_sheriff: str, all_players_hub: MsgHub, current_alive: list[ReActAgent]) -> None:
    """处理警长死亡时的警徽传递"""
    global sheriff, sheriff_has_badge, moderator, all_players_original_order
...
async def handle_tie_vote(
    ...
    # 让平票的玩家依次发言
    pk_agents = [agent for agent in current_alive if agent.name in tied_players]
    ...
    pk_agents = [agent for agent in current_alive if agent.name in tied_players]
    # PK投票时，只有非PK台的玩家可以投票
    voting_agents = [agent for agent in current_alive if agent.name not in tied_players]
...
def update_players(dead_players: list[str]) -> None:
    ...
    global werewolves, villagers, seer, hunter, witch, guard, wolf_king, current_alive, full_werewolves
...
async def main() -> None:
    ...
    # )
    global healing, poison, villagers, werewolves, seer, witch, hunter, guard, wolf_king, full_werewolves, first_explode_interrupted_election, first_day_candidates
    global current_alive, all_players_original_order, exploded_agent, dead_today
    # Create players
    ...
    for round_num in range(MAX_GAME_ROUND):
        ...
        async with MsgHub(
            ...
            for agent in hunter:
                ...
                await notify_hunter_poison_status(agent, is_poisoned)

            # Wolf King's turn - 夜晚只告知中毒状态，不能开枪
            for agent in wolf_king:
                # 告知狼王是否中毒（无论是否被击杀）
                is_poisoned = (poisoned_player == agent.name)
                await notify_wolf_king_poison_status(agent, is_poisoned)
            ...
            if round_num == 0 and not sheriff:
                await sheriff_election(all_players_hub, all_players_original_order)

            if round_num == 1 and first_explode_interrupted_election:
                await sheriff_election_vote(all_players_hub, current_alive)
            ...
            if len([_ for _ in dead_tonight if _]) > 0:
                ...
                # 处理夜晚死亡的遗言（首夜有遗言）
                if round_num == 0:
                    await handle_last_words(all_players_hub, dead_tonight)
            else:
                await all_players_hub.broadcast(
            ...
            for agent in gun_players:
                ...
                if (
                    ...
                    if NAME_TO_ROLE[agent.name] == "hunter":
                        ...
                        shot_player = await wolf_king_stage(agent)

            if shot_player:

                # 先更新死亡玩家列表
                update_players([shot_player])
                ...
                for agent in gun_players:
                    ...
                    if (
                        ...
                        if NAME_TO_ROLE[agent.name] == "hunter":
                            shot_player2 = await hunter_stage(agent)
                        elif NAME_TO_ROLE[agent.name] == "wolf_king":
                            shot_player2 = await wolf_king_stage(agent)

                if shot_player2:
                    update_players([shot_player2])
            ...
            if round_num == 0:
                if shot_player:
                    await handle_last_words(all_players_hub, [shot_player],  is_gunned=True)
                if shot_player2:
                    await handle_last_words(all_players_hub, [shot_player2], is_gunned=True)
            ...
            if round_num == 0:
                ...
                if sheriff:
                    ...
                    speaking_order = get_sheriff_based_speaking_order(current_alive, sheriff, sheriff_speaking_direction)
                ...
                if dead_tonight:
                    ...
                    if sheriff:
                        # 有警长，由警长决定从死左还是死右开始
                        await choose_sheriff_speaking_order_for_discussion(all_players_hub, sheriff, True)
                        speaking_order = get_death_based_speaking_order(current_alive, dead_tonight, sheriff_speaking_direction, all_players_original_order)
                    else:
                        # 没有警长，随机决定
                        import random
                        ...
                        speaking_order = get_death_based_speaking_order(current_alive, dead_tonight, is_clockwise, all_players_original_order)
                else:
                    # 平安夜时
                    if sheriff:
                        # 有警长，由警长决定从警左还是警右开始
                        await choose_sheriff_speaking_order_for_discussion(all_players_hub, sheriff, False)
                        speaking_order = get_sheriff_based_speaking_order(current_alive, sheriff, sheriff_speaking_direction)
                    else:
                        # 没有警长，基于时间决定
                        speaking_order = get_time_based_speaking_order(current_alive)
            ...
            if sheriff:
                ...
                if sheriff_agent:
                    ...
                    # 检查警长是否自爆
                    if await handle_self_explode(sheriff_agent, all_players_hub):
                        # 警长自爆了，直接进入黑夜
                        ...
                        # 处理警徽传递
                        await handle_sheriff_death(sheriff_agent.name, all_players_hub, current_alive)
            ...
            if voted_player is not None:
                await handle_last_words(all_players_hub, [voted_player], is_voted=True)
