...
async def handle_self_explode(agent: ReActAgent, all_players_hub: MsgHub, moderator: EchoAgent, NAME_TO_ROLE: dict) -> bool:
    """处理狼人自爆逻辑，返回是否自爆"""
...
async def choose_sheriff_speaking_order_for_discussion(all_players_hub: MsgHub, sheriff: str, has_death: bool,
                                                       moderator: EchoAgent, sheriff_speaking_direction: bool, current_alive: list[ReActAgent]) -> bool:
    """警长选择讨论发言顺序（有死亡或平安夜时）"""
...
async def handle_sheriff_death(dead_sheriff: str, all_players_hub: MsgHub, current_alive: list[ReActAgent] , sheriff : str, sheriff_has_badge: bool, moderator: EchoAgent, all_players_original_order: list[ReActAgent]) -> tuple:
    """处理警长死亡时的警徽传递"""
...
def update_players(dead_players: list[str], werewolves: list[ReActAgent], villagers: list[ReActAgent], seer: list[ReActAgent],hunter: list[ReActAgent],witch: list[ReActAgent], guard: list[ReActAgent], wolf_king: list[ReActAgent], current_alive: list[ReActAgent], full_werewolves: list[ReActAgent]) -> tuple:
...
async def create_player(role: str, NAME_TO_ROLE:  dict, ch_names: dict, moderator: EchoAgent) -> tuple:
...
async def main() -> None:
    ...
    for _ in range(4):
        a, NAME_TO_ROLE = await create_player("villager", NAME_TO_ROLE, ch_names, moderator)
        villagers.append(a)
    # werewolves = [await create_player("werewolf") for _ in range(3)]
    for _ in range(3):
        a, NAME_TO_ROLE = await create_player("werewolf", NAME_TO_ROLE, ch_names, moderator)
        werewolves.append(a)
    # seer = [await create_player("seer")]
    a, NAME_TO_ROLE = await create_player("seer", NAME_TO_ROLE, ch_names, moderator)
    seer.append(a)
    # witch = [await create_player("witch")]
    a, NAME_TO_ROLE = await create_player("witch", NAME_TO_ROLE, ch_names, moderator)
    witch.append(a)
    # hunter = [await create_player("hunter")]
    a, NAME_TO_ROLE = await create_player("hunter", NAME_TO_ROLE, ch_names, moderator)
    hunter.append(a)
    # guard = [await create_player("guard")]
    a, NAME_TO_ROLE = await create_player("guard", NAME_TO_ROLE, ch_names, moderator)
    guard.append(a)
    # wolf_king = [await create_player("wolf_king")]
    a , NAME_TO_ROLE = await create_player("wolf_king", NAME_TO_ROLE, ch_names, moderator)
    wolf_king.append(a)
    ...
    for round_num in range(MAX_GAME_ROUND):
        ...
        async with MsgHub(
            ...
            if dead_tonight:
                ...
                werewolves, villagers, seer, hunter, witch, guard, wolf_king, current_alive, full_werewolves =update_players(dead_tonight, werewolves, villagers, seer, hunter, witch, guard, wolf_king, current_alive, full_werewolves)
                ...
                if sheriff and sheriff in dead_tonight:
                    ...
                    sheriff, sheriff_has_badge = await handle_sheriff_death(dead_sheriff, all_players_hub, current_alive,sheriff, sheriff_has_badge, moderator, all_players_original_order)
            ...
            if shot_player:
                ...
                # 先更新死亡玩家列表
                werewolves, villagers, seer, hunter, witch, guard, wolf_king, current_alive, full_werewolves =update_players([shot_player], werewolves, villagers, seer, hunter, witch, guard, wolf_king, current_alive, full_werewolves)
                ...
                if shot_player2:
                    werewolves, villagers, seer, hunter, witch, guard, wolf_king, current_alive, full_werewolves =update_players([shot_player2], werewolves, villagers, seer, hunter, witch, guard, wolf_king, current_alive, full_werewolves)
            ...
            if dead_tonight_with_shots != dead_tonight:
                ...
                if sheriff and sheriff in dead_tonight_with_shots:
                    ...
                    sheriff, sheriff_has_badge = await handle_sheriff_death(dead_sheriff, all_players_hub,
                                                                            current_alive, sheriff, sheriff_has_badge,
            ...
            if voted_player is not None:
                ...
                werewolves, villagers, seer, hunter, witch, guard, wolf_king, current_alive, full_werewolves =update_players([voted_player], werewolves, villagers, seer, hunter, witch, guard, wolf_king, current_alive, full_werewolves)
