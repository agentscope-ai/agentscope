...
async def calculate_profitability_ratios(
    ...
    try:
        ...
        # 提取关键数据
        revenue = _get_latest_value(income_statement, "Revenue")
        cost_of_revenue = _get_latest_value(income_statement, "Cost of Revenue")
        gross_profit = _get_latest_value(income_statement, "Gross Profit")
        operating_income = _get_latest_value(income_statement, "Operating Income")
        net_income = _get_latest_value(income_statement, "Net Income")
        ...
        if revenue and revenue > 0:
            ...
            ratios["revenue_growth"] = _calculate_growth_rate(
                income_statement, "Revenue"
            )
        
        # ROE计算
        shareholders_equity = _get_latest_value(
        ...
        # 分析和评级
        analysis = _analyze_profitability(ratios)
        
        result = {
            "ratios": ratios,
            "analysis": analysis,
            "benchmark_comparison": _compare_with_benchmarks(
                ratios, "technology"
            ),
...
async def calculate_solvency_ratios(
    ...
    try:
        ...
        # 提取关键数据
        total_assets = _get_latest_value(balance_sheet, "Total Assets")
        current_assets = _get_latest_value(balance_sheet, "Current Assets")
        total_liabilities = _get_latest_value(balance_sheet, "Total Liabilities")
        current_liabilities = _get_latest_value(balance_sheet, "Current Liabilities")
        ...
        # 速动比率（假设存货是流动资产的一部分）
        inventory = _get_latest_value(balance_sheet, "Inventory")
        ...
        # 利息保障倍数
        operating_income = _get_latest_value(income_statement, "Operating Income")
        interest_expense = _get_latest_value(income_statement, "Interest Expense")
        if operating_income and interest_expense and interest_expense > 0:
            ratios["interest_coverage"] = operating_income / interest_expense
        
        # 分析和评级
        analysis = _analyze_solvency(ratios)
...
async def calculate_efficiency_ratios(
    ...
    try:
        ...
        # 应收账款周转率（假设有应收账款数据）
        accounts_receivable = _get_latest_value(
        ...
        # 分析和评级
        analysis = _analyze_efficiency(ratios)
...
async def trend_analysis(
    historical_data: Dict[str, Any], years: int = 3
) -> ToolResponse:
    """
    趋势分析
    ...
    try:
        ...
        # 收入趋势
        revenue_data = _extract_trend_data(historical_data, "Revenue")
        if revenue_data:
            trend_results["revenue"] = _analyze_trend(revenue_data, "收入")
        
        # 净利润趋势
        net_income_data = _extract_trend_data(historical_data, "Net Income")
        if net_income_data:
            trend_results["net_income"] = _analyze_trend(net_income_data, "净利润")
        
        # 资产趋势
        assets_data = _extract_trend_data(historical_data, "Total Assets")
        if assets_data:
            trend_results["assets"] = _analyze_trend(assets_data, "总资产")
        
        # 负债趋势
        liabilities_data = _extract_trend_data(historical_data, "Total Liabilities")
        if liabilities_data:
            trend_results["liabilities"] = _analyze_trend(liabilities_data, "总负债")
        
        # 整体趋势分析
        overall_analysis = _overall_trend_analysis(trend_results)
...
def _get_latest_value(data: Dict, key: str) -> Optional[float]:
    """获取数据的最新值"""
...
def _calculate_growth_rate(data: Dict, key: str) -> Optional[float]:
    """计算增长率"""
    if key in data and isinstance(data[key], dict):
...
def _analyze_profitability(ratios: Dict[str, float]) -> Dict[str, str]:
    """分析盈利能力"""
    analysis = {}
...
def _analyze_solvency(ratios: Dict[str, float]) -> Dict[str, str]:
    """分析偿债能力"""
    analysis = {}
...
def _analyze_efficiency(ratios: Dict[str, float]) -> Dict[str, str]:
    """分析运营效率"""
    analysis = {}
...
def _compare_with_benchmarks(
    ratios: Dict[str, float], industry: str
) -> Dict[str, str]:
    """与行业基准比较"""
    industry_benchmarks = {
        "technology": {
...
def _extract_trend_data(data: Dict, key: str) -> List[Tuple[str, float]]:
    """提取趋势数据"""
    trend_data = []
...
def _analyze_trend(data: List[Tuple[str, float]], metric_name: str) -> Dict:
    """分析单个指标的趋势"""
    if len(data) < 2:
...
def _overall_trend_analysis(trend_results: Dict) -> str:
    """整体趋势分析"""
