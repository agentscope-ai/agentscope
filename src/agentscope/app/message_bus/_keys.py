...
class MessageBusKeys:  # pylint: disable=too-many-public-methods
    ...
    def channel_liveness(cls, channel_id: str) -> str:
        ...
        Fields are node ids, values that node's ``ChannelStatus``. Read
        by the API to report status from a replica that holds no
        connection; entries expire so a dead node stops appearing.
        """
