...
Holding the connection is all it does. Outbound never comes back
here: a reply is plain REST, so the node running the agent builds its
own client and delivers it directly.
"""
import asyncio
import socket
...
class ChannelLifecycleDispatcher:
    ...
    async def _periodic(self) -> None:
        ...
        connection — after the worker split that is every API replica.
        """
        while True:
