...
async def deliver_to_inbox(
    ...
    *,
    ...
    """Push a payload to a session inbox and trigger a run for it.

    The trigger is unconditional: a run already under way claims the
    entry on its next reasoning step, and the extra trigger then finds
    the inbox empty and returns without reasoning.
