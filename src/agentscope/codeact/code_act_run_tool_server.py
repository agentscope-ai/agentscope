...
class CodeActRunToolServer():
    ...
    def __init__(self, port:int, toolkit: Toolkit):
        ...
        if not port or port <= 0:
            raise ValueError('CodeActRunToolServer missing a positive port argument')
        if not toolkit:
    ...
    async def start(self):
        ...
        async def run_tool(requst: RunToolRequest) -> JSONResponse:
            ...
            try:
                ...
                logger.info(f'/run_tool, request_id:{request_id}, tool_name:{requst.tool_name}, tool_args:{requst.tool_args}')
                ...
                async for tool_response in result:
                    if tool_response.content and tool_response.content[0]['text']:
                ...
                try:
                    ...
                    if tool_call_result_type == 'set':
                        ...
                        tool_call_result_type = type(tool_call_result).__name__
                        logger.info(f'/run_tool, request_id:{request_id}, casted set to list, {tool_call_result}')


                    logger.info(f'/run_tool, request_id:{request_id}, casted result_text |{result_text}| to |{tool_call_result}| of type {tool_call_result_type}')
                    return JSONResponse(content={"result": tool_call_result, "type": tool_call_result_type}, status_code=200)
                except Exception:
                    logger.warning(f'/run_tool, request_id:{request_id}, return text type directly |{result_text}|', exc_info=True)
                    return JSONResponse(content={"result": result_text, "type": "str"}, status_code=200)
        ...
        self.code_act_server_thread = Thread(target=uvicorn.run, args=[app], kwargs={'host': host, 'port': self._port, 'access_log': True}, daemon=True)
