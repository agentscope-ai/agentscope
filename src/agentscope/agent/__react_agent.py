...
class ReActAgent(ReActAgentBase):
    ...
    async def _gather_acting(self, tool_calls: Sequence[ToolUseBlock]) -> dict | None:
        """Gather the acting results from multiple tool calls.

        Args:
            tool_calls (`list[ToolUseBlock]`):
                The list of tool use blocks generated in the reasoning step.

        Returns:
            `dict | None`:
                The structured output if it's generated and verified in the
                finish function call, otherwise return None.
        """
        external_tools, futures = [], []
        for tool_call in tool_calls:
            if self.toolkit.is_external_tool_function(tool_call):
    ...
    async def reply(
        ...
        while True:
            ...
            if n_iter > 0 or enter_phase == "reasoning":
                # Prepare hint messages and tool choice for the reasoning step
                exit_reply, tool_choice = await self._prepare(
                    n_iter,
                    msg,
                    structured_model,
                    reasoning_msg,
            ...
            structured_output = await self._gather_acting(
                reasoning_msg.get_content_blocks('tool_use')
            )
