...
from .._utils._common import _base_model_to_json, _base_model_from_json
...
class ReActAgent(ReActAgentBase):
    ...
    def __init__(
        ...
        # 同一时间，这个智能体类只会awaiting一种状态，要么是等待用户确认，要么是等待工具执行结果，不会同时等待两者，所以这里不需要区分两者的id，直接放在一起即可
        # 但是需要一个字段能够区分当前等待的状态是用户确认还是工具执行结果，以便在_reply函数中正确处理输入消息
        self._awaiting_state: Literal[
            "user_confirmation",
            "tool_execution",
        ] | None = None
        self._awaiting_tool_calls: list[str] = []

        # The pending tool calls that are waiting to be executed after
        # external tool results are received
        self._pending_tool_calls: list[ToolUseBlock] = []
        ...
        self.register_state("_awaiting_state")
        self.register_state("_awaiting_tool_calls")
        self.register_state("_pending_tool_calls")
        self.register_state(
            "_required_structured_model",
            custom_to_json=_base_model_to_json,
            custom_from_json=_base_model_from_json,
        )
        self.register_state("_current_iters")
    ...
    async def reply(
        ...
        """
        reasoning_msg = None
        if self._awaiting_state:
            reasoning_msg = await self._handle_awaiting_messages(msg)
        ...
        tool_choice = None
        while self._current_iters < self.max_iters:
            # -------------- Memory compression --------------
            await self._compress_memory_if_needed()

            # -------------- The reasoning process --------------
            if not reasoning_msg:
                reasoning_msg = await self._reasoning(tool_choice)

            # -------------- Acting with HITL --------------
            acting_msg = self._acting_with_hitl(reasoning_msg)
            ...
            if self._required_structured_model:
                ...
                if structured_outputs:
                    ...
                    if reasoning_msg.has_content_blocks("text"):
                        # Re-use the existing text response if any to avoid
                        # duplicate text generation
                        reply_msg = Msg(
                            self.name,
                            reasoning_msg.get_content_blocks("text"),
                            "assistant",
                ...
                elif not reasoning_msg.has_content_blocks("tool_use"):
                ...
                reasoning_msg.metadata = structured_output
                reply_msg = reasoning_msg
                break
    ...
    async def _acting_with_hitl(self, reasoning_msg: Msg) -> Msg | None:
        ...
        if self.parallel_tool_calls:
            to_confirm = [
                _
                for _ in tool_calls
                if self.toolkit.require_user_confirmation(
                    _.get("name"),
                )
                and not _.get("confirmed", False)
            ]

            if to_confirm:
                self._pending_tool_calls = [
                    _ for _ in tool_calls if _ not in to_confirm
                ]
                ...
                return Msg(
                    ...
                    to_confirm,
                    "assistant",
                    generate_reason=GenerateReason.AWAITING_USER_CONFIRMATION,
                )

            to_external = [
                _
                for _ in tool_calls
                if self.toolkit.is_external_tool_function(_.get("name"))
            ]

            if to_external:
                self._pending_tool_calls = [
                    _ for _ in tool_calls if _ not in to_external
                ]
                ...
                return Msg(
                    ...
                    to_external,
                    "assistant",
                    generate_reason=GenerateReason.AWAITING_TOOL_EXECUTION,
                )

            # Parallel execution
            results = await asyncio.gather(
                *[self._acting(tool) for tool in tool_calls],
            )
            # Check for structured output from finish function
            if results:
                reasoning_msg.metadata = results[0]
            ...
            for i in range(len(tool_calls)):
                ...
                if self.toolkit.require_user_confirmation(
                    ...
                    self._pending_tool_calls = tool_calls[i + 1 :]

                    self._awaiting_state = "user_confirmation"
                    self._awaiting_tool_calls = [tool_call.get("id")]
                ...
                if self.toolkit.is_external_tool_function(
                    tool_call.get("name"),
                ):
                    self._pending_tool_calls = tool_calls[i + 1 :]

                    self._awaiting_state = "tool_execution"
                    self._awaiting_tool_calls = [tool_call.get("id")]
            ...
            if result:
                reasoning_msg.metadata = result
                return reasoning_msg

        return None

    async def _handle_awaiting_messages(
        self,
        msg: Msg | list[Msg] | None,
    ) -> list[ToolUseBlock]:
        """Extract pending tool calls from input message(s).
        ...
        Returns:
            `list[ToolUseBlock]`:
                Tool calls that are confirmed and ready to execute directly
        ...
        # 首先确认输入消息能够完全覆盖当前 await 的状态，如果不能抛出错误；这个阶段不会对
        # memory 做任何修改，支持检查，避免部分确认导致状态混乱的情况

        approved_or_modified, rejected = [], []
        execution_results = []
        if self._awaiting_state == "user_confirmation":
            for m in msgs:
                if m.generate_reason in [
                    GenerateReason.USER_CONFIRMATION_APPROVED,
                    GenerateReason.USER_CONFIRMATION_MODIFIED,
                ]:
                    approved_or_modified.append(
                        m.get_content_blocks("tool_use"),
                    )

                elif (
                    m.generate_reason
                    == GenerateReason.USER_CONFIRMATION_REJECTED
                ):
                    rejected.append(m.get_content_blocks("tool_use"))

            received_confirmation_ids = {
                _["id"] for _ in approved_or_modified + rejected if "id" in _
            }

            if received_confirmation_ids == self._awaiting_tool_calls:
                # All awaiting tool calls are confirmed or rejected
                self._awaiting_state = None
                self._awaiting_tool_calls = []

            else:
                raise ValueError(
                    f"The input message(s) must contain the confirmation "
                    f"results of all awaiting tool calls with ids: "
                    f"{self._awaiting_tool_calls}, but received ids: "
                    f"{received_confirmation_ids}.",
                )

        elif self._awaiting_state == "tool_execution":
            for m in msgs:
                if m.generate_reason == GenerateReason.TOOL_EXECUTION_RESULT:
                    execution_results.extend(
                        m.get_content_blocks("tool_result"),
                    )

            received_execution_ids = {
                _["id"] for _ in execution_results if "id" in _
            }

            if received_execution_ids == set(self._awaiting_tool_calls):
                # All awaiting tool calls are executed and results received
                self._awaiting_state = None
                self._awaiting_tool_calls = []

            else:
                raise ValueError(
                    f"The input message(s) must contain the execution results "
                    f"of all awaiting tool calls with ids: "
                    f"{self._awaiting_tool_calls}, but received ids: "
                    f"{received_execution_ids}.",
                )

        # 检查结束后，开始实际执行 awaiting 后的处理逻辑
        confirmed_tool_calls = []
        if self._awaiting_state == "user_confirmation":
            # User confirmation results processing
            confirmed_tool_calls.extend(approved_or_modified)

            # Record the execution results into the memory
            await self.memory.add(
                Msg(
                    "system",
                    [
                        ToolResultBlock(
                            ...
                            name=_["name"],
                            output=(
                                "<system-info>User rejected the tool call: "
                                f"'{_['name']}' with input {_['input']}."
                                f"</system-info>"
                            ),
                        )
                        for _ in rejected
        ...
        elif self._awaiting_state == "tool_execution" and execution_results:
            # Record the tool execution into the memory
            await self.memory.add(
                Msg("system", execution_results, "system"),
            )

        # Note: self._pending_messages 不为空的时候，当前一定正在处理awaiting的消息
        # 考虑从self._pending_messages里面拿出来可以执行的消息

        return confirmed_tool_calls
