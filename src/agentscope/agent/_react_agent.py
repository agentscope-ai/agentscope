...
from ..message import GenerateReason
...
class ReActAgent(ReActAgentBase):
    ...
    async def reply(  # pylint: disable=too-many-branches
        ...
        for i in range(self.max_iters):
            # Decide the current intention
            tool_choice = self._prepare_reasoning()
            ...
            external_tools = [_ for _ in tool_calls if self.toolkit.is_external_tool_function(_)]
            ...
            # -------------- The acting process --------------
            futures = [
                self._acting(_) for _ in msg_reasoning.get_content_blocks(
                    "tool_use"
                )
            ]
    ...
    async def _acting(self, tool_call: ToolUseBlock) -> dict | None:
        ...
        """
        if self.toolkit.is_external_tool_function(tool_call["name"]):
