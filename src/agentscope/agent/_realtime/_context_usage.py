...
class ContextUsageTracker:
    ...
    @property
    def provenance(self) -> Literal[UsageProvenance.PROVIDER, UsageProvenance.ESTIMATED] | None:
        if self._observation is None:
            return None
    ...
    def note_local_append(self, count: int = 1) -> None:
        ...
        self._observation.stale_reason = (
            f"{self._local_appends_since_observation} local appends since observation"
        )
