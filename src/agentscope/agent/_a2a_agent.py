...
class A2aAgentConfig:
	...
	supported_transports: list[TransportProtocol | str] = field(default_factory=list)
	...
	push_notification_configs: list[PushNotificationConfig] = field(default_factory=list)
	...
	additional_transport_producers: dict[str, TransportProducer] = field(default_factory=dict)
...
class A2aAgent(AgentBase):
	...
	async def reply(
		...
		try:
			...
			logger.info(f"[{self.__class__.__name__}] Communication cancelled by user")
			raise
			...
			if response_msg is None:
				logger.warning(f"[{self.__class__.__name__}] No response received from remote agent")
				response_msg = Msg(
					name=self.name,
					content=[TextBlock(
						type="text",
	...
	def _construct_msg_from_task_status(self, task: Task) -> Msg:
		...
		if task.status.message:
			...
			response_msg = self._convert_a2a_message_to_msg(task.status.message)
			...
			if isinstance(response_msg.content, str):
				...
				response_msg.content = [state_info_block, TextBlock(type="text", text=response_msg.content)]
	...
	def _convert_task_artifacts_to_msg(self, task: Task) -> Msg | None:
		...
		for artifact in task.artifacts:
			...
			all_content_blocks.append(TextBlock(type="text", text=artifact_info))
	...
	def _convert_a2a_message_to_msg(self, a2a_message: A2AMessage) -> Msg:
		...
		for part in a2a_message.parts:
			...
			msg_id = part_metadata.get("_agentscope_msg_id") if part_metadata else None
			...
			if msg_id:
				...
				if msg_id not in msg_groups:
					msg_groups[msg_id] = {
						"msg_id": msg_id,
						"msg_source": part_metadata.get("_agentscope_msg_source", ""),
						"content_blocks": []
					}
	...
	def _convert_part_to_content_block(self, part: Part) -> ContentBlock | None:
		...
		if isinstance(part_root, TextPart):
			...
			part_metadata = part_root.metadata if hasattr(part_root, 'metadata') else None
			block_type = part_metadata.get("_agentscope_block_type") if part_metadata else None
	...
	def _convert_data_part_to_block(self, data_part: DataPart) -> ContentBlock:
		...
		"""
		# Check for AgentScope metadata
		part_metadata = data_part.metadata if hasattr(data_part, 'metadata') else None
		block_type = part_metadata.get("_agentscope_block_type") if part_metadata else None
		...
		# Case 2: ToolResult block
			...
			tool_name = part_metadata.get("_agentscope_tool_name")
			tool_id = part_metadata.get("_agentscope_tool_call_id")
			tool_output = data_part.data.get("_agentscope_tool_output")  # Data.data._agentscope_tool_output → ToolResult.output
		...
		# Case 3: No AgentScope metadata - serialize to JSON and create TextBlock
			...
			data_json = json.dumps(data_part.data, ensure_ascii=False, indent=2)
		...
		for msg in msgs:
			...
			if isinstance(msg.content, str):
				...
				if msg.content.strip():  # Filter empty strings
					merged_parts.append(
						Part(root=TextPart(text=msg.content, metadata=part_metadata))
					)
	...
	def _convert_content_block_to_part(self, block: ContentBlock) -> Part | None:
		...
		# ToolResult block -> DataPart
			...
			return Part(root=DataPart(
				data={"_agentscope_tool_output": tool_output},  # ToolResult.output → Data.data._agentscope_tool_output
				metadata={
	...
	async def _ensure_ready(self) -> None:
		...
		for transport_label, transport_producer in self._agent_config.additional_transport_producers.items():
			self._a2a_client_factory.register(transport_label, transport_producer)
	...
	def _extract_a2a_client_config(self) -> ClientConfig:
		...
		a2a_client_config = ClientConfig(
			...
			supported_transports=self._agent_config.supported_transports or [TransportProtocol.jsonrpc],
