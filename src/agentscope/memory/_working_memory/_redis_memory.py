...
class RedisMemory(MemoryBase):
    ...
    def __init__(
        ...
        Args:
            ...
            redis_url (`str`, default to `"redis://localhost:6379/0"`):
                The Redis connection URL. Used if `redis_client` is not
                provided.
            redis_client (`Redis | None`, optional):
                An optional Redis client instance. If provided, it will be
                used instead of creating a new connection. Defaults to `None`.
