...
- 必须遵守：Docs-first → todo → 批准 → 执行；核心契约（Agent/ReAct 六步、Pipeline、Toolkit/MCP、Formatter、Model、Memory、MsgHub、Tracing）不得随意修改；质量门槛 `ruff check src`、`pytest` 为硬要求。
...
2) 生成以下内联区块（紧凑可执行）：
   ...
   - **ACCEPTANCE**：列出硬性通过项（ruff check src、pytest 目标、SOP/README/CLAUDE.md 同步、行为验证用例）。
