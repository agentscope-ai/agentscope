...
- Checks：每次改动后必须运行 `pre-commit run --files $(git ls-files 'src/**')`（包含 black/ruff/flake8/pylint/mypy）与相应 `pytest` 目标。Ruff 仅检测但必须运行；在合并前需清零告警。
...
3) Verify：每个小步后运行 ruff/mypy/pytest 并记录要点（通过/需要修复）。
