...
for round in range(3):
    pro_response = await pro_agent(debate_topic)
    con_response = await con_agent(debate_topic, pro_response)
```
...
async def async_agent_workflow():
    # 并行执行独立任务
    weather_task = weather_agent.run_async("杭州天气")
    news_task = news_agent.run_async("今日新闻")

    weather, news = await asyncio.gather(weather_task, news_task)

    # 汇总结果
    summary = await summary_agent.run(f"天气：{weather}，新闻：{news}")
    return summary
```
