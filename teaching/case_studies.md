...
- 基于Actor模型的分布式机制
- 内置ReAct Agent、DialogAgent、DeepResearchAgent等多种智能体类型
...
**核心功能**:
- Docker/Kubernetes/Serverless部署支持
- 安全沙箱隔离（Python、Shell、Browser、FileSystem等）
- OpenTelemetry全链路追踪
- A2A协议支持
...
**代码示例**:
```python
from agentscope import MsgHub, AgentBase

# 创建MsgHub并加入Agent
with MsgHub(agents=[agent_a, agent_b]) as hub:
    # 广播消息给所有参与者
    hub.broadcast(msg)
```
...
| 模式 | 描述 | 适用场景 |
|------|------|----------|
| Sequential | 顺序执行，输出作为输入 | 线性任务流程 |
| Parallel | 并行执行，汇总结果 | 独立子任务 |
| Conditional | 条件路由选择分支 | 动态决策流程 |
...
| 类别 | 亮点 | 适用场景 |
|------|------|----------|
| **消息中枢** | MsgHub实现透明可追溯的消息通信 | 需要多Agent协作的场景 |
| **流水线编排** | Pipeline支持多种协作模式 | 复杂任务分解执行 |
| **RAG集成** | 灵活的外部知识接入 | 知识密集型应用 |
| **语音交互** | 实时双向通信能力 | 客服、助手类应用 |
| **人在回路** | Hook机制实现运行时干预 | 生产环境安全保障 |
| **分布式部署** | 基于Actor模型的自动并行 | 大规模生产系统 |
| **A2A协议** | Agent间标准化通信 | 跨系统协作 |
| **沙箱安全** | 多类型执行隔离 | 代码/浏览器安全执行 |
