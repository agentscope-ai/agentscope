...
消息结构包含 `name`（名称）、`role`（角色）和 `content`（内容或ContentBlock列表），以及可选的 `metadata` 字段用于存储额外信息。`timestamp` 和 `id` 由系统自动生成。
...
长期记忆模块位于 `src/agentscope/memory/_long_term_memory/`，提供基于不同存储后端的长期记忆实现：
...
memory = Mem0LongTermMemory(
    user_id="user123",
    api_key="your-mem0-api-key"  # 可选配置
)

# ReMe方法的个人长期记忆
from agentscope.memory import ReMePersonalLongTermMemory
memory = ReMePersonalLongTermMemory(user_id="user123")
```
