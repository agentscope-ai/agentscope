...
**Hook机制**：
```python
# 使用 register_instance_hook 注册实例钩子
agent.register_instance_hook(
    hook_type="pre_reply",  # 钩子类型：pre_reply, post_reply, pre_observe, post_observe
    name="correction",      # 钩子名称
    callback=my_correction_logic  # 回调函数
)
```
