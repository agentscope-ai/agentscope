...
```python
async with MsgHub(participants=[a1, a2], enable_auto_broadcast=True) as hub:
    result = await a1(Msg("user", "hello", "user"))
    # result 会被广播给 a2 吗？
```

**预期输出/行为**：
当 `enable_auto_broadcast=True` 时，AgentBase 的 `_broadcast_to_subscribers` 会触发自动广播，消息经过 `MsgHub._announced_host` 广播。`broadcast()` 只影响手动调用 `hub.broadcast()` 的场景。
