...
> **注意**: 上方表结构为概念示意，简化了部分 ORM 关系定义（如 `relationship()` 的 `back_populates` 完整配置、`cascade` 设置、索引策略等）。完整的生产级 ORM 模型请参考源码 `_sqlalchemy_memory.py` 获取精确实现。
...
# 创建向量存储和知识库（需要 embedding_store 和 embedding_model 两个必需参数）
# MilvusLiteStore 需要 uri, collection_name, dimensions 三个必需参数
embedding_store = MilvusLiteStore(
    uri="./milvus_data.db",
...
核心属性：`embedding_store`（VDBStoreBase 向量存储实例）和 `embedding_model`（EmbeddingModelBase 嵌入模型实例）。两者均为构造函数必需参数。`retrieve` 是面向用户的高级接口，支持 query + limit + score_threshold 参数，内部自动调用 `self.embedding_model(...)` 生成查询向量后通过 `self.embedding_store.search()` 执行相似度检索。
...
DocMetadata 存储 `content`（TextBlock/ImageBlock/VideoBlock）、`doc_id`、`chunk_id`、`total_chunks`。检索时 `score`（相似度分数）和 `chunk_id` 会被填充返回。
...
class ChromaDBStore(VDBStoreBase):
    """ChromaDB 向量存储实现示例。
    注意：VDBStoreBase 没有 __init__，子类自行管理客户端和 collection。"""

    def __init__(self, collection_name: str, dimensions: int, **kwargs):
        import chromadb
    ...
    async def add(self, documents: list[Document], **kwargs) -> None:
        """实现 VDBStoreBase 的抽象方法 add()。"""
        self.collection.add(
            ids=[doc.id for doc in documents],
            documents=[doc.metadata.content.text for doc in documents],
            metadatas=[{"chunk_id": doc.metadata.chunk_id} for doc in documents],
        )
    ...
    async def search(
        ...
        **kwargs,
        ...
        """实现 VDBStoreBase 的抽象方法 search()。"""
        results = self.collection.query(
            query_embeddings=[query_embedding],
            n_results=limit,
        )
        return [
            Document(metadata=DocMetadata(content=TextBlock(text=doc), chunk_id=int(id_)))
