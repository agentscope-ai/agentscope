...
async def debate(topic: str, rounds: int = 3):
    ...
    # 收集各方回应
    pro_result = await pro_agent(Msg(
        name="user",
        content=f"请就辩题'{topic}'发表正方观点",
        role="user"
    ))
    con_result = await con_agent(Msg(
        name="user",
        content=f"请就辩题'{topic}'发表反方观点",
        role="user"
    ))
    ...
    for i in range(rounds - 1):
        ...
        pro_response = await pro_agent(Msg(
            name="user",
            content=f"反方观点：{con_result.content}，请反驳",
            role="user"
        ))
        # 反方回应正方
        con_response = await con_agent(Msg(
            name="user",
            content=f"正方观点：{pro_result.content}，请反驳",
            role="user"
        ))
