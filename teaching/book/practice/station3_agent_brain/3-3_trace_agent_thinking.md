...
async def multi_turn_demo():
    # 第一轮
    user_input = Msg(name="user", content="帮我查北京天气，然后告诉我穿什么", role="user")
    ...
    # 内部：决定调用穿衣建议Tool
    result2 = await agent(Msg(name="user", content="继续", role="user"))
    # Agent内部记住了之前的天气信息
```
...
async def debug_agent():
    ...
    # 打印最终回复
    response = await agent(Msg(name="user", content="北京到上海多远", role="user"))
