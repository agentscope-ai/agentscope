...
- **统一入口**: `__call__()` 方法作为外部调用统一入口，调用 `reply()` 并将结果广播给订阅者（`observe()` 和 `print()` 由子类在 `reply()` 内部调用）
...
# 以下为 AgentBase 自身初始化的属性：

# 标识
self.id: str                        # 自动生成的唯一 ID

# Hook 相关
self.supported_hook_types: list     # 支持的钩子类型列表（6 种）
self._instance_pre_reply_hooks      # 实例级 pre_reply 钩子
self._instance_post_reply_hooks     # 实例级 post_reply 钩子
# ... 以及 pre_print/post_print/pre_observe/post_observe 钩子

# 状态相关
self._subscribers: dict             # 订阅者字典 {hub_name: [agent, ...]}
self._reply_task: asyncio.Task      # 当前回复任务（用于中断）

# 注意：name、sys_prompt、model、memory、tools、max_iters 等属性
# 由子类（如 ReActAgent、UserAgent）在各自的 __init__ 中初始化
```
...
关键区别：`reply()` 是"主动响应"，`observe()` 是"被动接收"。`__call__()` 仅调用 `reply()` 并广播结果；`observe()` 由子类在 `reply()` 内部自行调用（如 ReActAgent 的 `_reasoning` 循环中处理广播消息时）。
...
- 通过 `register_class_hook(hook_type, hook_name, hook)` 注册到类级别或 `register_instance_hook(hook_type, hook_name, hook)` 注册到实例级别
- `hook_type` 支持的值：`"pre_reply"`、`"post_reply"`、`"pre_print"`、`"post_print"`、`"pre_observe"`、`"post_observe"`
...
# 注册
AgentBase.register_class_hook("post_reply", "auto_compress", auto_compress_hook)
```
...
| 设计模式 | 应用位置 | 说明 |
|----------|----------|------|
| **Template Method** | `AgentBase.reply()` | 抽象方法，子类直接重写 `reply()` 实现各自逻辑 |
| **Strategy** | Hook 注册机制 | 运行时注入前置/后置处理逻辑 |
| **Observer** | `_broadcast_to_subscribers` | 发布-订阅消息通知 |
| **Decorator** | `_AgentMeta` 元类 | 自动包装 reply/observe/print 方法 |
| **Memento** | `register_state` | 自动持久化关键状态属性 |
| **Chain of Responsibility** | ReActAgent reason→act 循环 | 推理和行动交替执行 |

## 小结

| 组件 | 关键能力 |
|------|----------|
| AgentBase | 模板方法 reply + Hook 系统 + 消息订阅 |
| ReActAgentBase | reason/act 循环框架 |
| ReActAgent | 完整实现：LLM 推理 + 工具调用 + 记忆管理 |
